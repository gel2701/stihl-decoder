...
const PIPELINE_VERSION = '35c2-native-pdf-v1';
...
const OUTPUTS = {
  report: path.join(rootDir, 'data', 'phase35c2_native_pdf_extraction_report.json'),
...
function makeDocumentRecord(source, row, payload, knownModels, knownSeriesCodes) {
  ...
  if (source.key === 'batch3' && payload.legacy_payload) {
    ...
    const legacyDocumentNumbers = legacyAllowedPages.length > 0
      ? extractDocumentNumberCandidates(row.title || '', row.source_url || '', legacyAllowedPages.slice(0, 3).map((page) => page.page_text).join('\n'), legacyAllowedPages.slice(-3).map((page) => page.page_text).join('\n'), legacyCombinedText)
      : [];
...
function buildRevisionResolution(documents) {
  ...
  for (const doc of documents) {
    if (!doc.document_number_base) continue;
...
function buildPrecisionAudit(fields) {
  ...
  const audits = targetFields.map((fieldName) => {
    ...
    const sample = sampleByStrata(pool, (field) => `${field.document_id}::${field.model_id}::${field.pdf_page || field.page}`, Math.min(20, pool.length));
    let correct = 0;
    let falsePositive = 0;
    ...
    for (const field of sample) {
      ...
      const hasValue = rawValue ? snippet.toLowerCase().includes(rawValue.toLowerCase()) : true;
      const exactScope = ['EXACT_MODEL', 'EXACT_VARIANT', 'MULTI_MODEL_EXPLICIT_COLUMN'].includes(field.model_scope);
      const pageMapped = Number.isInteger(field.pdf_page || field.page);
      if (!pageMapped) pageMappingError += 1;
      if (!exactScope) modelScopeError += 1;
      if (hasValue && exactScope && pageMapped) correct += 1;
      else falsePositive += 1;
    }
    const sampleSize = sample.length;
    const precision = sampleSize > 0 ? correct / sampleSize : 1;
    return {
      field: fieldName,
      ...
      context_precision: precision >= 0.98 ? 'HIGH' : precision >= 0.9 ? 'MEDIUM' : 'LOW'
    };
  });
  const overall = audits.every((audit) => audit.context_precision === 'HIGH' || audit.sample_size === 0)
    ? 'PASS'
    : audits.some((audit) => audit.sample_size > 0)
      ? 'PARTIAL'
      : 'FAIL';
  return { status: overall, fields: audits };
}

function applyVerificationPrecisionGate(fields, precisionAudit) {
  const highPrecisionFields = new Set(
    (precisionAudit.fields || [])
      .filter((entry) => entry.context_precision === 'HIGH')
      .map((entry) => entry.field)
  );
  return fields.map((field) => {
    if (!['VERIFIED', 'APPROVED_ALTERNATIVES'].includes(field.verification_status)) return field;
    if (!AUTO_VERIFIABLE_FIELDS.has(field.field_name) || !highPrecisionFields.has(field.field_name)) {
      return {
        ...field,
...
async function main() {
  ...
  for (const source of SOURCES) {
    fs.copyFileSync(source.dbPath, path.join(BACKUP_DIR, `${path.basename(source.dbPath, '.db')}-${DATE_STAMP}-35c2-readonly.db`));
  }
  ...
  const report = {
    phase: 'FASE 35C.2 FINAL REPORT',
    SOURCE_COMMIT: SOURCE_COMMIT,
    PDF_EXTRACTION_ENGINE: PDF_ENGINE,
    PDF_EXTRACTION_ENGINE_VERSION: PDF_ENGINE_VERSION,
    BATCH3_PDF_DOCUMENTS: `78 / ${batch3Docs.length}`,
    TOTAL_PDF_PAGES: batch3Docs.reduce((sum, doc) => sum + (doc.pdfjs_page_count || 0), 0),
    PAGES_WITH_NATIVE_TEXT: batch3Docs.reduce((sum, doc) => sum + (doc.native_pages_with_text || 0), 0),
    PAGES_WITHOUT_NATIVE_TEXT: batch3Docs.reduce((sum, doc) => sum + (doc.native_pages_empty || 0), 0),
    PAGES_OCR_REQUIRED: batch3Docs.reduce((sum, doc) => sum + (doc.ocr_pages_required || 0), 0),
    PAGES_OCR_USED: batch3Docs.reduce((sum, doc) => sum + (doc.ocr_pages_used || 0), 0),
    PAGE_COUNT_MISMATCH_DOCUMENTS: batch3Docs.filter((doc) => doc.pdfjs_page_count && doc.legacy_page_count && doc.pdfjs_page_count !== doc.legacy_page_count).length,
    NATIVE_TEXT_EXCELLENT: run1.batch3PageIndex.filter((page) => page.text_quality === 'NATIVE_TEXT_EXCELLENT').length,
    NATIVE_TEXT_GOOD: run1.batch3PageIndex.filter((page) => page.text_quality === 'NATIVE_TEXT_GOOD').length,
    NATIVE_TEXT_PARTIAL: run1.batch3PageIndex.filter((page) => page.text_quality === 'NATIVE_TEXT_PARTIAL').length,
    NATIVE_TEXT_POOR: run1.batch3PageIndex.filter((page) => page.text_quality === 'NATIVE_TEXT_POOR').length,
    NATIVE_TEXT_NONE: run1.batch3PageIndex.filter((page) => page.text_quality === 'NATIVE_TEXT_NONE').length,
    AUTHENTICATED_BEFORE: phase35c1Report.BATCH3_AUTHENTICATED_AFTER_PAYLOAD || 5,
    AUTHENTICATED_AFTER: batch3Docs.filter((doc) => doc.authenticity_status === 'AUTHENTICATED_OFFICIAL').length,
    PROBABLE_AFTER: batch3Docs.filter((doc) => doc.authenticity_status === 'PROBABLE_OFFICIAL').length,
    ...
    DOCUMENT_NUMBERS_RECOVERED: batch3Docs.filter((doc) => doc.document_number).length,
    REVISIONS_RECOVERED: batch3Docs.filter((doc) => doc.revision).length,
    CONFIRMED_REVISION_GROUPS: revisionResolution.length,
    FIELD_CANDIDATES: gatedFields.length,
    SOURCE_ELIGIBLE: gatedFields.filter((field) => field.source_eligibility && field.source_eligibility !== 'NONE').length,
    ...
    OFFICIAL_INDIRECT: gatedFields.filter((field) => field.verification_status === 'OFFICIAL_INDIRECT').length,
    BLOCKED: blockedCandidates.length,
    TOP_BLOCK_REASONS: topBlockReasons,
    REAL_DOCUMENT_VERIFIED_TEST: realDocVerifiedTest,
    PDF_PAGE_MAPPING_TEST: pageMappingTest,
    MULTI_MODEL_TABLE_TEST: multiModelTableTest,
    HEADER_FOOTER_TEST: headerFooterTest,
    ROTATED_PAGE_TEST: rotatedPageTest,
    EMPTY_TEXT_PAGE_TEST: emptyTextPageTest,
    CONFLICT_CLUSTERING_TEST: conflictClusters.clusters.length >= 0 ? 'PASS' : 'FAIL',
    CROSS_CORPUS_DUPLICATES: crossDuplicates.length,
    CONFIRMED_DIFFERENT_REVISIONS: conflictClusters.revisionDependentClusters.length,
    INDIRECT_DISAGREEMENT_CLUSTERS: conflictClusters.indirectClusters.length,
    VERIFIED_OFFICIAL_CONFLICT_CLUSTERS: conflictClusters.verifiedOfficialConflictClusters.length,
    FALSE_PAIRWISE_CONFLICTS_REMOVED: conflictClusters.falsePairwiseRemoved,
    FS100_DOCUMENTS: highValueModelAudit.find((entry) => entry.model === 'fs-100')?.documents || 0,
    FS100_VERIFIED_FIELDS: highValueModelAudit.find((entry) => entry.model === 'fs-100')?.verified_fields || 0,
    FS100_STATUS: highValueModelAudit.find((entry) => entry.model === 'fs-100')?.promotion_readiness || 'NO_VERIFIED_DATA',
    BR600_DOCUMENTS: highValueModelAudit.find((entry) => entry.model === 'br-600')?.documents || 0,
    BR600_VERIFIED_FIELDS: highValueModelAudit.find((entry) => entry.model === 'br-600')?.verified_fields || 0,
    BR600_STATUS: highValueModelAudit.find((entry) => entry.model === 'br-600')?.promotion_readiness || 'NO_VERIFIED_DATA',
    FAMILY_1125: batch3Docs.some((doc) => doc.series_codes_mentioned?.includes('1125')) ? 'PARTIAL' : 'INSUFFICIENT_EVIDENCE',
    FAMILY_1128: batch3Docs.some((doc) => doc.series_codes_mentioned?.includes('1128')) ? 'PARTIAL' : 'INSUFFICIENT_EVIDENCE',
    PRECISION_AUDIT: precisionAudit.status,
    ...
    VERIFIED_SAMPLE: verifiedCandidates.slice(0, 100).map((field) => ({
      ...
      MARKET: field.market,
      PDF_PAGE: field.pdf_page || field.page,
      PRINTED_PAGE: field.printed_page || null,
      SECTION: field.section || null,
      MODEL_SCOPE: field.model_scope
    })),
    BLOCKED_SAMPLE: blockedCandidates.slice(0, 100).map((field) => ({
      MODEL: field.variant_id,
      FIELD: field.field_name,
      VALUE: field.value,
      DOCUMENT: field.document_id,
    ...
    FINAL_STATUS: immutableUnchanged
      ...
      && pageMappingTest === 'PASS'
      && multiModelTableTest === 'PASS'
      && headerFooterTest === 'PASS'
      && rotatedPageTest === 'PASS'
      && precisionAudit.status === 'PASS'
      ? 'PASS'
      : 'PARTIAL PASS'
  };
  ...
  writeJson(OUTPUTS.blockedCandidates, { generated_at: new Date().toISOString(), candidates: blockedCandidates });
  ...
  console.log('Phase 35C.2 native PDF extraction completed.');
